...
def get_parallel_sentences(base_file_paths):
    print("Read files -")
    ...
    for i in range(len(base_file_paths)):
        base_file_path = base_file_paths[i]

        filename = os.path.join(base_file_path, "1" + "_origin_sentences.txt")
        origins = read_from_file(filename)
        origin_groups.append(origins)

        filename = os.path.join(base_file_path, "1" + "_reference_sentences.txt")
        references = read_from_file(filename)
        reference_groups.append(references)

        filename = os.path.join(base_file_path, "1" + "_predicted_sentences.txt")
...
if __name__ == "__main__":
    project_name = "bart-newsela-check"  # newsela-transformer-bert-weights
    ...
    if tracking_active:
        ...
        experiment.log_other("evaluation_files_path", base_file_paths)

    origin_groups, reference_groups, prediction_groups = get_parallel_sentences(base_file_paths)
