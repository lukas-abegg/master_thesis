...
class EncoderRNN(nn.Module):
    def __init__(self, vocab_size, embedding_dim, pos_vocab_size, pos_embedding_dim, hidden_size, n_layers=1,
                 embedding=None, embeddingPOS=None, dropout=0.3):
        ...
        self.rnn = nn.LSTM(embedding_dim + pos_embedding_dim, hidden_size, num_layers=n_layers, batch_first=True,
                           bidirectional=True)
...
class EditDecoderRNN(nn.Module):
    ...
    def execute_batch(self, batch_symbol, batch_input, batch_lm_state):
        ...
        for i in range(bsz):
            elem = self.execute(unbind_symbol[i], unbind_input[i],
                                (unbind_h[i].view(1, 1, -1), unbind_c[i].view(1, 1, -1)))
    ...
    def beam_forwad_step(self, decoder_input_edits, hidden_edits, hidden_words, org_ids, encoder_outputs_org,
                         counter_for_keep_del, beam_size=5):
        ...
        while t < tt:
            ...
            for b in range(len(best_k_seqs)):
                ...
                if seq[-1].item() == STOP_ID or check:
                    ...
                    decoder_input_k, hidden_edits_k, hidden_words_k, prob_k, counter_for_keep_del_k = self.beam_forwad_step(
                        seq[-1],
